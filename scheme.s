...
;r9 holds numerator, r8 denominator
reduce_fraction:
	push rbp
	mov rbp, rsp
	...
	push r14
	push r15
	mov r14, r9; r14 gets numerator
	mov r15, r8
	mov rsi, r9; r14 gets numerator
	mov rdi, r8


	L_reduce_fraction_call_gcd:
	call gcd;				now r14 holds greatest common divisor

	common_divisor:
	mov rax, rsi
	mov rbx, rdi
	xor rdx,rdx; 				rdx holds remainder
	call my_div; rax = rax/r14
	mov r9, rax
	mov rax, rbx; rax = rbx
	call my_div; rbx = rbx/r14
	mov r8, rax

	call fix_sign

	L_reduce_fraction_exit:
	pop r15
	pop r14
	leave
	ret
...
;rax contain left number
;rbx contain right number
my_div:
	push rbp
	mov rbp, rsp
	L_reduce_fraction_check_first_arg:
	cmp rax, 0
	jl L_reduce_fraction_numerator_neg

	L_reduce_fraction_numerator_pos:
	idiv r14
	jmp my_div_exit

	L_reduce_fraction_numerator_neg:
	neg rax
	idiv r14
	neg rax
...
;r9 holds numerator
;r8 holds denominator
fix_sign:
	push rbp
	mov rbp, rsp

	L_fix_sign_check_numerator_sign:
	cmp r9, 0
	jl L_fix_sign_numerator_neg_check_denominator

	L_fix_sign_numerator_pos_check_denominator:
	cmp r8, 0
	jl L_fix_sign_numerator_pos_denominator_neg

	L_fix_sign_numerator_pos_denominator_pos:
	jmp fix_sign_exit

	L_fix_sign_numerator_neg_check_denominator:
	cmp r8, 0
	jnl fix_sign_exit

	L_fix_sign_numerator_neg_denominator_neg:
	neg r8
	neg r9
	jmp fix_sign_exit

	L_fix_sign_numerator_pos_denominator_neg:
	neg r8
	neg r9
	jmp fix_sign_exit
...
%define MAKE_SYMBOL(lit) (((lit - start_of_data) << TYPE_BITS) | T_SYMBOL)

%macro MAKE_RT_SYMBOL 1
mov rax,%1
...
;;; MAKE_MALLOC_LITERAL_FRACTION target-addrees, numerator, denominator
...
%macro GET_CAR_ADDR 1
	DATA_UPPER %1
	add %1, start_of_data
%endmacro

%macro GET_CDR_ADDR 1
	DATA_LOWER %1
	add %1, start_of_data
%endmacro
...
%macro MAKE_LITERAL_CLOSURE 3
	push rax
	push rbx
	mov rax, %1;										target to rax
	mov qword [rax], %2;								env to [rax] 
	sub qword [rax],start_of_data;						envAddress=envAdress-start_of_data
	shl qword [rax], ((WORD_SIZE - TYPE_BITS) >> 1);	???
...
;second arg cannot be rax!
%macro MULTIPLY 2
	push rax
	push rbx
	mov rax, %1
	mov rbx, %2
	push rdx;		mul destroys rdx
	mul rbx
	pop rdx
	mov %1,rax
	pop rax
